...
def merge_tag(tag, files, output_dir, log_dir):
    ...
    """
    final_path = os.path.join(output_dir, f"insitu-observations-surface-land_monthly_{tag}.pq")
...
def merge_final_pq_parallel(parent_input_dir, output_dir, max_workers=None):
    ...
    file_tags = {}
    pattern = re.compile(r"monthly_(\d{4}_\d{2})\.pq$")
    for f in pq_files:
    ...
    if failed:
        fail_log = os.path.join(output_dir, "failed_merge_log_mnth.csv")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Chunked memory-efficient parallel merge of monthly Parquet files")
