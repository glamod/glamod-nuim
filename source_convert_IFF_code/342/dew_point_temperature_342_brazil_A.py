...
OUTDIR = "D:/brazil_inmet/DataA_all_years_IFF/dew_point_temperature/"
os.chdir("D:/brazil_inmet/DataA/")
extension = 'csv'
...
for filename in all_filenames:
   ...
   #df[['Year',"Month", "Day"]] = df['Data'].str.split('/',expand=True)#odd 2019
    df[["Hour","zeros"]] = df["HORA (UTC)"].str.split(':',expand=True)
    #df[['Hour',"zeros"]] = df['Hora UTC'].str.split(' ',expand=True)#2019 odd
    #df['Hour'] = df['HORA (UTC)']
    df["Original_observed_value"] = df["TEMPERATURA DO PONTO DE ORVALHO (°C)"]
    df["Observed_value"] = df["TEMPERATURA DO PONTO DE ORVALHO (°C)"]
    ...
    df3.to_csv(outname+"_dew_point_temperature_342.psv", index=False, sep="|")
