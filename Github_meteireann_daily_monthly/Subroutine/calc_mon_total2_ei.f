...
c     AJ_Kettle, Nov09/2017
      ...
      SUBROUTINE calc_mon_total2_ei(f_ndflag,
...
c     running values before day check
      REAL                :: f_matrix_integrator(300,12)
      REAL                :: f_matrix_maxrun(300,12)
      REAL                :: f_matrix_minrun(300,12)

c     values after day check; flags applied
      REAL                :: f_matrix_truedaycnt(300,12)
      REAL                :: f_matrix_average(300,12)
      REAL                :: f_matrix_total(300,12)
...
c************************************************************************
c      print*,'just inside calc_mon_total2_ei'
...
c     Initial i_matrix_truedaycnt
      DO j=1,300
       DO k=1,12 
        ...
        f_matrix_maxrun(j,k)    =0.0
        f_matrix_minrun(j,k)    =1000000.0
...
c       Resolve running min/max values
        f_matrix_maxrun(i_loc_year,i_loc_month) =
     +    MAX(f_matrix_maxrun(i_loc_year,i_loc_month),
     +        f_vec_rain_mm(i))
        f_matrix_minrun(i_loc_year,i_loc_month) =
     +    MIN(f_matrix_minrun(i_loc_year,i_loc_month),
     +        f_vec_rain_mm(i))
...
c       Initialize month values to -999.0
        f_matrix_average(j,k)=f_ndflag
        f_matrix_total(j,k)  =f_ndflag
        f_matrix_min(j,k)    =f_ndflag
        f_matrix_max(j,k)    =f_ndflag
...
c         Calculate average
     ...
     +       f_matrix_integrator(j,k)/f_matrix_truedaycnt(j,k)
           f_matrix_total(j,k)=f_matrix_integrator(j,k)
           f_matrix_max(j,k)  =f_matrix_maxrun(j,k)
           f_matrix_min(j,k)  =f_matrix_minrun(j,k)
          ENDIF
...
c************************************************************************
c************************************************************************
c     Construct vector time series with gaps of month data
...
c       Initialize float variables ndflags
        f_monrec_common_nseconds(ii) =f_ndflag
        f_monrec_value(ii)           =f_ndflag
        f_monrec_total(ii)           =f_ndflag
        f_monrec_max(ii)             =f_ndflag
        f_monrec_min(ii)             =f_ndflag
     ...
     +      i_matrix_monthflag(j,k).EQ.1) THEN
         ...
         s_monrec_common_timezone5(ii)='+0000'       !hardwire time zone

         f_monrec_value(ii) =f_matrix_average(j,k)
         f_monrec_total(ii) =f_matrix_total(j,k)
