...
# -*- coding: utf-8 -*-
"""
Subroutines for daily QFF to CDM conversion scripts
...
def add_data_policy(var_frame, policy_frame):
    ...
    policy_frame = policy_frame.astype(str)

    # merge
    var_frame = policy_frame.merge(var_frame, on=['primary_station_id_2'], how="inner")

    # -------------------------------
    # SAFE COLUMN HANDLING (FIX)
    # -------------------------------
    if 'data_policy_licence_x' in var_frame.columns:
        var_frame['data_policy_licence'] = var_frame['data_policy_licence_x']
    else:
        var_frame['data_policy_licence'] = var_frame['data_policy_licence']
    ...
    # height fix
    if 'height_of_station_above_sea_level_x' in var_frame.columns:
        var_frame['height_of_station_above_sea_level'] = var_frame['height_of_station_above_sea_level_x']

    var_frame['height_of_station_above_sea_level'] = (
        var_frame['height_of_station_above_sea_level']
        .astype(str)
        .str.replace('.0', '', regex=False)
    )
