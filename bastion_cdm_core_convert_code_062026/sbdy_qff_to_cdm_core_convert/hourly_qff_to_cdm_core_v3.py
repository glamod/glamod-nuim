...
SUB_DAILY_QC_FLAGS = {
    ...
    "f" : "2,",  # Frequent
    "u" : "3,",  # diurnal
    "d" : "4,",  # Distribution (monthly)
    "b" : "5,",  # distribution (all)
    "x" : "6,",  # excess streak proportion
    "k" : "7,",  # repeating streaks
    "c" : "8,",  # Climatological,
    "t" : "9,",  # Timestamp
    "s" : "10,", # Spike
    "h" : "11,", # High flag rate
   ...
   # To start at begining of files
    for filename in all_filenames:
       ...
       # dfmslp['observation_value'] = dfmslp['observation_value'].map(int)
        ...
        dfwd = h_utils.apply_wind_measurement_codes(dfwd, ["", "N", "N-Normal", "C", "C-Calm", "V", "V-Variable", "9", "9-Missing"])
       ...
       # dfwd['observation_value'] = dfwd['observation_value'].astype(str).apply(lambda x: x.replace('.0', ''))
        ...
        dfws = h_utils.apply_wind_measurement_codes(dfws, ["", "N", "N-Normal", "C", "C-Calm", "V", "V-Variable", "9", "9-Missing"])
