...
for filename in all_filenames:
    ...
    df["Observed_value"] = df["Temperature (°C)"]
    df["Original_observed_value"] = df["Temperature (°C)"]
    try:
    ...
    try:
        ...
        outname = os.path.join(OUTDIR1, ID)
        a = df2['Station_ID'].unique()
        print (a)
        #with open(filename, "w") as outfile:
        df2.to_csv(outname+"_temperature_387.psv", index=False, sep="|")
    except:
        continue