...
c     AJ_Kettle, 05Dec2018
      ...
      SUBROUTINE find_distinct_timesteps2(l_lines_rgh,l_lines,
...
c************************************************************************
      print*,'just inside find_distinct_timesteps'
      ...
      DO j=1,l_lines-1
       IF (s_arch_date_yyyymmdd(j).NE.s_arch_date_yyyymmdd(j+1)) THEN 
c       Update list of distinct dates
        ii=ii+1
        s_distinct_date_yyyymmdd(ii)=s_arch_date_yyyymmdd(j+1)
        i_vec_en(ii-1)=j
        i_vec_st(ii)  =j+1
