...
c     AJ_Kettle, Dec7/2017

      SUBROUTINE export_observation_subdaily2(f_ndflag,s_directory_root,
...
c     Variables used in subroutine
      ...
      CHARACTER(LEN=3)    :: s_code(7)
      CHARACTER(LEN=2)    :: s_value_signif_code(7)
      ...
      CHARACTER(LEN=4)    :: s_orig_units               !39
      ...
      INTEGER             :: i_keep
c************************************************************************
      print*,'just entered export_observation_subdaily'

      print*,'ft_sd_pres_hpa=',(ft_sd_pres_hpa(i),i=1,5)
      print*,'ft_sd_slpres_hpa=',(ft_sd_slpres_hpa(i),i=1,5)

      print*,'f_ndflag=',f_ndflag

      print*,'f_duration_s=',(f_duration_s(i),i=1,5)
      print*,'s_basis_hght_single=',s_basis_hght_single
      print*,'s_basis_lat_single=',s_basis_lat_single
      print*,'s_basis_lon_single=',s_basis_lon_single
c      CALL SLEEP(4)


c     Variables from ICARUS document
c                       export            original        freq
c     1. acc ppt        ft_sd_ppt_mm                      3Xday
c     2. air_temp       ft_sd_airt_k      ft_sd_airt_c    3Xday
c     3. slpres         ft_sd_slpres_hpa  ft_sd_pres_hpa  3Xday
c     4. wspd           ft_sd_wspd_ms     ft_sd_wspd_bft  3Xday
c     5. wdir           ft_sd_wdir_deg    ft_sd_wdir_code 3Xday
c(     6. dewp           ft_sd_dewp_k      ft_sd_vprs_hpa  3Xday)
c     6. relh           ft_sd_relh_pc                     3Xday                    
c     7. snowfall       ft_snow_daytot_cm                 1Xday
c************************************************************************
c     Find date & time
...
c     Declare variable codes
      ...
      s_code(4) ='107'   !wspd
      s_code(5) ='106'   !wdir 
      s_code(6) ='38'    !relative humidity
      s_code(7) ='55'    !snowfall
      ...
      s_value_signif_code(6)='12'
      s_value_signif_code(7)='13'
c*****
c     Declare source_id
      s_source_id='82'
c*****
      s_title=s_basis_wigos_full_single//'_observation.txt'
...
c     Line-stepper for output file
      DO j=1,j_sd
c      Cycle through first 6 subdaily variables
       DO k=1,7

        i_day_mod=MOD(j+2,3)
        i_day_ref=-999
        IF (i_day_mod.EQ.0) THEN 
         i_day_ref=INT(FLOAT(j)/3.0)+1
        ENDIF
c        print*,'i_day_mod=',i_day_mod,'i_day_ref=',i_day_ref,j
c        CALL SLEEP(1)
c*******
          s_year =st_sd_year(j)
...
c         1.  Observation_id
          ...
          s_observation_id=s_basis_wigos_full_single//'-'//
     +     s_year//'-'//s_month//'-'//s_day//'-'//
     +     s_hour//':'//s_minute//'-'//
     +     TRIM(s_code(k))//'-'//TRIM(s_value_signif_code(k))

c          i_test=k
c          WRITE(s_test8,'(i8)') i_test
c          s_observation_id=ADJUSTL(s_test8)
c*******
c         2.  report_id
...
c         10 minute for all point variables
          IF (k.EQ.2.OR.k.EQ.3.OR.k.EQ.4.OR.k.EQ.5.OR.k.EQ.6) THEN 
           s_obs_duration='600'
          ENDIF

          IF (k.EQ.7) THEN 
           s_obs_duration='86400'       !day for snow accumulation
          ENDIF
c*******
c         7. longitude
...
c         15.s_observation_value 
          ...
          IF (k.EQ.1) THEN
           f_test=ft_sd_ppt_mm(j)
           IF (f_test.EQ.f_ndflag.OR.
     +         f_duration_s(j).EQ.f_ndflag) THEN 
            s_observation_value='NULL'
           ENDIF
           IF (.NOT.(f_test.EQ.f_ndflag.OR.
     +               f_duration_s(j).EQ.f_ndflag)) THEN 
          ...
          IF (k.EQ.7) THEN
c          Initialize value to NULL
           s_observation_value='NULL'
c          Act only if i_day_ref correct
           IF (i_day_ref.GT.0) THEN
            f_test=ft_snow_daytot_cm(i_day_ref)
            IF (f_test.EQ.f_ndflag) THEN 
             s_observation_value='NULL'
            ENDIF
            IF (f_test.NE.f_ndflag) THEN 
...
c         16.s_value_significance                !CDM Table 116
          ...
          ENDIF
          IF (k.EQ.7) THEN
           s_value_significance='13'            !snowfall, day accumulation
          ENDIF
c*******
...
c         18.s_units
          ...
          ENDIF
          IF (k.EQ.5) THEN
           s_units='320'           !wdir_deg
          ENDIF
          IF (k.EQ.6) THEN
           s_units='300'           !relh_pc
          ENDIF
          IF (k.EQ.7) THEN
           s_units='055'           !snowfall
          ENDIF
c*******
c         19.s_code_table
          s_code_table='NULL'
...
c         20.s_conversion_flag          !CDM table 99
          ...
          ENDIF
          IF (k.EQ.3) THEN
           s_conversion_flag='2'     !slpres_hpa
          ENDIF
          IF (k.EQ.4) THEN
           s_conversion_flag='0'     !wspd_ms
          ENDIF
          IF (k.EQ.5) THEN
           s_conversion_flag='3'     !wdir_deg
          ENDIF
          IF (k.EQ.6) THEN
...
c         32.s_numer_precision (number of decimal places)
          ...
          ENDIF
          IF (k.EQ.4) THEN
           s_numer_precision='0'     !wspd_ms
          ENDIF
          IF (k.EQ.5) THEN
           s_numer_precision='0'     !wdir_deg
          ENDIF
          IF (k.EQ.6) THEN
           s_numer_precision='0'     !relh_pc
          ENDIF
          IF (k.EQ.7) THEN
           s_numer_precision='0'     !snoacc_cm
          ENDIF
c*******
...
c         38.s_orig_precision
          ...
          ENDIF
          IF (k.EQ.3) THEN
           s_orig_precision='1'     !mslp_hpa
          ENDIF
          IF (k.EQ.4) THEN
           s_orig_precision='0'     !wspd_ms
          ENDIF
          IF (k.EQ.5) THEN
           s_orig_precision='0'     !wdir_deg
          ENDIF
          IF (k.EQ.6) THEN
           s_orig_precision='0'     !relh_pc
          ENDIF
          IF (k.EQ.7) THEN
           s_orig_precision='0'     !snoacc_cm
          ENDIF
c*******
c         39.s_orig_units
          IF (k.EQ.1) THEN  
          ...
          ENDIF
          IF (k.EQ.3) THEN
           s_orig_units='530'   !stnpres_hpa
          ENDIF
          IF (k.EQ.4) THEN
           s_orig_units='NULL'  !wspd_ms
          ENDIF
          IF (k.EQ.5) THEN
           s_orig_units='NULL'  !wdir_deg
          ENDIF
          IF (k.EQ.6) THEN
...
c         40.s_original_value
          ...
          IF (k.EQ.2) THEN
           ...
           ENDIF
          ENDIF
          IF (k.EQ.3) THEN                         !pres_mb
           f_test=ft_sd_pres_hpa(j)
           IF (f_test.EQ.f_ndflag) THEN 
            s_original_value='NULL'
           ENDIF
           IF (f_test.NE.f_ndflag) THEN
           ...
           ENDIF
          ENDIF
          IF (k.EQ.4) THEN                         !wspd_bf
           f_test=ft_sd_wspd_bft(j)
           IF (f_test.EQ.f_ndflag) THEN 
            s_original_value='NULL'
           ENDIF
           IF (f_test.NE.f_ndflag) THEN
           ...
           ENDIF
          ENDIF
          IF (k.EQ.5) THEN                          !wspd_ptcompass
           f_test=ft_sd_wdir_code(j)
           IF (f_test.EQ.f_ndflag) THEN 
            s_original_value='NULL'
           ENDIF
           IF (f_test.NE.f_ndflag) THEN
...
c          Act only if i_day_ref correct
           IF (i_day_ref.GT.0) THEN
            f_test=ft_snow_daytot_cm(i_day_ref)                 !snoacc_cm
            IF (f_test.EQ.f_ndflag) THEN 
             s_original_value='NULL'
            ENDIF
            IF (f_test.NE.f_ndflag) THEN
...
c         41.s_conversion_method
          ...
          ENDIF
          IF (k.EQ.4) THEN
           s_conversion_method='5'         !wspd 
          ENDIF
          IF (k.EQ.5) THEN
           s_conversion_method='4'         !wdir 
          ENDIF
          IF (k.EQ.6) THEN
...
c      s_code(4) ='107'   !wspd
c      s_code(5) ='106'   !wdir 
c      s_code(6) ='38'    !relative humidity
c      s_code(7) ='55'    !snowfall
c*******
       GOTO 21
        ...
        print*,'s_obs_hght_above_sfc'//TRIM(s_obs_hght_above_sfc)//'xx'
        ...
        print*,'s_value_significance'//TRIM(s_value_significance)//'xx'
        ...
        print*,'s_location_precision'//TRIM(s_location_precision)//'xx'
        ...
        print*,'s_method_uncertainty'//TRIM(s_method_uncertainty)//'xx'
...
c        print*,'s_traceability'//TRIM(s_traceability)//'xx'
c        print*,'s_advanced_qc'//TRIM(s_advanced_qc)//'xx'
c        print*,'s_adv_uncertainty'//TRIM(s_adv_uncertainty)//'xx'
c        print*,'s_adv_homogen'//TRIM(s_adv_homogen)//'xx'
c        print*,'s_multiple_source'//TRIM(s_multiple_source)//'xx'
...
c************************************************************************
      print*,'just leaving export_observation_subdaily'
c      CALL SLEEP(2)
