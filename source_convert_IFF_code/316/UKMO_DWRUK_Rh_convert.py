...
df = pd.read_csv("DWR_UKMO_DWRUK_FRANKFURT_19081115-19101231_rh.tsv",skiprows=12, sep='\t')
#extract the first 11 rows of the header to a df
df1=pd.read_csv("DWR_UKMO_DWRUK_FRANKFURT_19081115-19101231_rh.tsv", nrows=11,sep='\t', squeeze=True)
df2=df1.iloc[:,0:2]
...
with open('output.csv') as fin:    
    ...
    for row in csvin:
        ...
        if cat not in outputs:
            fout = open ('{}_relative_humidity_316.psv'.format(cat), "w", newline = "")##change source ID
