...
c     AJ_Kettle, 11Mar2020
      ...
      SUBROUTINE process_files_prelim20210521(
     +  s_date_st,s_time_st,
     +  f_ndflag,d_ndflag,
     +  s_dir_usaf_filelist,
...
c*****
c      INTEGER             :: l_scoutput_rgh
c      INTEGER             :: l_scoutput
c      CHARACTER(LEN=20)   :: s_scinput2_primary_id(l_scoutput_rgh)
c      CHARACTER(LEN=2)    :: s_scinput2_record_number(l_scoutput_rgh)
c      CHARACTER(LEN=20)   :: s_scinput2_secondary_id(l_scoutput_rgh)
c      CHARACTER(LEN=50)   :: s_scinput2_station_name(l_scoutput_rgh)
c      CHARACTER(LEN=10)   :: s_scinput2_longitude(l_scoutput_rgh)
c      CHARACTER(LEN=10)   :: s_scinput2_latitude(l_scoutput_rgh)
c      CHARACTER(LEN=10)   :: s_scinput2_elevation_m(l_scoutput_rgh)
c      CHARACTER(LEN=1)    :: s_scinput2_policy_license(l_scoutput_rgh)
c      CHARACTER(LEN=3)    :: s_scinput2_source_id(l_scoutput_rgh)
c*****
...
c     Metadata variables
      ...
      INTEGER             :: i_cnt_good
c************************************************************************
      print*,'just entered process_files_prelim20210521'

c      STOP 'process_files_prelim20210521'

c      print*,'s_vec_stnlist=',(TRIM(s_vec_stnlist(i)),i=1,20)

c      print*,'s_scinput_primary_id,1=',
c     +  (TRIM(s_scinput_primary_id(i)),i=1,20)
c      print*,'s_scinput_primary_id,2=',
c     +  (TRIM(s_scinput2_primary_id(i)),i=1,20)
c*****
c     Readin last index
      CALL readin_lastfile_index(s_dir_outfile_lastfile,
     +  s_last_netplat,i_lastindex,i_start)

      print*,'s_last_netplat=',TRIM(s_last_netplat)
      print*,'i_lastindex=',i_lastindex
...
c       IF (TRIM(s_networktype).EQ.'WMO'.OR.
c     +     TRIM(s_networktype).EQ.'ICAO'.OR.
c     +     TRIM(s_networktype).EQ.'AFWA'.OR.
c     +     TRIM(s_networktype).EQ.'CMANS') THEN
...
c     +  l_scinput_rgh,l_scinput,
c     +  s_scinput_primary_id,s_scinput_station_name,
c     +  s_scinput_longitude,s_scinput_latitude,s_scinput_elevation_m,

c     +  s_scsingle_primary_id,s_scsingle_station_name,
c     +  s_scsingle_longitude,s_scsingle_latitude,s_scsingle_elevation_m,
c     +  l_occur1)
...
c      print*,'s_metasingle_name=',TRIM(s_metasingle_name)
...
c      print*,'i_0good_1bad_2zero=',i_0good_1bad_2zero
c      STOP 'process_files_prelim20210521'
...
cc      Call subroutine to find max wspd in each year
c       CALL find_max_year_wspd4(f_ndflag,
c     +  s_dir_outfile_yearstats,
c     +  s_dir_outfile_yearstats_sift,
c     +  s_vec_networktype(1),s_vec_platformid(1),
c     +  l_rgh_datalines,l_data,
c     +  d_vec_jtime,s_vec_date_dd_mm_yyyy,s_vec_time_hh_mm_ss,
c     +  f_vec_windspeed_ms,
c     +  i_vec_triplet_index,
c     +  s_metasingle_platformid,s_metasingle_networktype,
c     +  s_metasingle_name,s_metasingle_st,s_metasingle_co,
c     +  s_metasingle_lat,s_metasingle_lon,s_metasingle_elev)
...
c      (in case program crashes, this defines starting point)
     ...
     +   s_filename,s_networktype,s_platformid,i)
      ...
      ENDDO
c*****
      print*,'just leaving process_files_prelim20210521'
