...
c     Jan14/2018: modified timezone from 5 to 3 char
c     Jan14/2018: changed duration from 600s to 3600s for rainfall 
 
      SUBROUTINE export_header_subday2(f_ndflag,s_directory_root,
     +  l_datalines,
...
c************************************************************************
      print*,'just inside export_header_subday'
...
c         print*,'s_date_single=',s_date_single
c         print*,'s_time_single=',s_time_single
...
c        27. report_meaning_of_time_stamp (1=beginning, 2=end; 3=middle)
         s_meaning_timestamp='2'               !2 for synoptic
c*******
...
c        29. report_duration
         s_report_duration='3600'       !longest duration for 1h rainfall accumulation
c*******
...
c*******
         i_keep=0
         IF (f_vec_airt_k(j).EQ.f_ndflag.AND.
     +       f_vec_rain_mm(j).EQ.f_ndflag.AND.
     +       f_vec_wspd_ms(j).EQ.f_ndflag.AND.
     +       f_vec_slpr_hpa(j).EQ.f_ndflag.AND.
     +       f_vec_relh_pc(j).EQ.f_ndflag) THEN
           i_reject=i_reject+1
         ENDIF         
         IF (.NOT.(f_vec_airt_k(j).EQ.f_ndflag.AND.
     +       f_vec_rain_mm(j).EQ.f_ndflag.AND.
     +       f_vec_wspd_ms(j).EQ.f_ndflag.AND.
     +       f_vec_slpr_hpa(j).EQ.f_ndflag.AND.
     +       f_vec_relh_pc(j).EQ.f_ndflag)) THEN
...
c************************************************************************
      print*,'just leaving export_header_subday'
