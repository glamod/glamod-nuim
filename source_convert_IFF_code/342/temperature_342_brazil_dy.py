...
OUTDIR = "D:/brazil_inmet/DataDaily_M_all_years_IFF/temperature"
os.chdir("D:/brazil_inmet/Daily_manual")
extension = '*'
...
for filename in all_filenames:
    ...
    df['Hour'] = df['HR_MEDICAO']/100
    df['Latitude'] = df["VL_LATITUDE"].round(3)
    df["Longitude"] = df["VL_LONGITUDE"].round(3)
    df["Elevation"] = ""
    df["Original_observed_value"] = df["TEMP_HORA"]
    df["Observed_value"] = df["TEMP_HORA"]
    ...
    df2.to_csv(outname+"_temperature_342.psv", index=False, sep="|")
