...
c     modified Dec8/2017: no header lines if all data bad
c     modified Dec9/2017: 3 header lines per day, last col changed, reduced time res

      SUBROUTINE export_header_subdaily2(f_ndflag,s_directory_root,
...
c     Variables used in subroutine
      ...
      INTEGER             :: i_reject,i_retain
      INTEGER             :: i_keep
c      INTEGER             :: i_k1_retain,i_k1_reject
c      INTEGER             :: i_k2_retain,i_k2_reject
...
c     Line-stepper for output file
      DO j=1,j_sd
        ...
        i_day_mod=MOD(j,3)
c        i_day_ref=-999
c        IF (i_day_mod.EQ.0) THEN 
c         i_day_ref=INT(FLOAT(j)/3.0)+1
c        ENDIF
c        print*,'i_day_mod=',i_day_mod,'j=',j
c        CALL SLEEP(1)

c       DO k=1,2      !k=1 for instaneous measurements; k=2 for ppt accumulations
c*******
...
c        For first obs of day: us 86400
         IF (i_day_mod.EQ.1) THEN 
          s_report_duration='86400' 
         ENDIF
c        For 2nd & 3rd obs of day: use calculated duration
         IF (.NOT.(i_day_mod.EQ.1)) THEN 
...
c          s_report_duration='600'    !hardwire report duration

c         IF (k.EQ.1) THEN
c          s_report_duration='600'
c         ENDIF

c         IF (k.EQ.2) THEN
c          IF (f_duration_s(j).EQ.f_ndflag) THEN
c           s_report_duration='NULL'
c          ENDIF
c          IF (f_duration_s(j).NE.f_ndflag) THEN
cc          Convert report duration from float to string
c           i_test=NINT(f_duration_s(j))
c           WRITE(s_test8,'(i8)') i_test
c           s_report_duration=ADJUSTL(s_test8)
c          ENDIF
c         ENDIF
c*******
...
c         s_source_record_id='NULL'
         ...
         s_source_record_id=
     +    TRIM(s_source_id)//'-'//
     +    TRIM(s_basis_wigos_full_single)//'-'//
     +    TRIM(s_report_timestamp)

c*******
c        Count number lines retained & cut

c        New rejection criteria
         i_keep=0
         IF (ft_sd_ppt_mm(j).EQ.f_ndflag.AND.
     +       ft_sd_airt_k(j).EQ.f_ndflag.AND.
     +       ft_sd_slpres_hpa(j).EQ.f_ndflag.AND.
     +       ft_sd_wspd_ms(j).EQ.f_ndflag.AND.
     +       ft_sd_wdir_deg(j).EQ.f_ndflag.AND.
     +       ft_sd_relh_pc(j).EQ.f_ndflag) THEN
           i_reject=i_reject+1
         ENDIF         
         IF (.NOT.(ft_sd_ppt_mm(j).EQ.f_ndflag.AND.
     +       ft_sd_airt_k(j).EQ.f_ndflag.AND.
     +       ft_sd_slpres_hpa(j).EQ.f_ndflag.AND.
     +       ft_sd_wspd_ms(j).EQ.f_ndflag.AND.
     +       ft_sd_wdir_deg(j).EQ.f_ndflag.AND.
...
cc        Case: all instantaneous variables
c         IF (k.EQ.1) THEN
c          IF (ft_sd_airt_k(j).EQ.f_ndflag.AND.
c     +        ft_sd_slpres_hpa(j).EQ.f_ndflag.AND.
c     +        ft_sd_wspd_ms(j).EQ.f_ndflag.AND.
c     +        ft_sd_wdir_deg(j).EQ.f_ndflag.AND.
c     +        ft_sd_relh_pc(j).EQ.f_ndflag) THEN
c           i_k1_retain=i_k1_retain+1
c          ENDIF
c          IF (.NOT.(ft_sd_airt_k(j).EQ.f_ndflag.AND.
c     +        ft_sd_slpres_hpa(j).EQ.f_ndflag.AND.
c     +        ft_sd_wspd_ms(j).EQ.f_ndflag.AND.
c     +        ft_sd_wdir_deg(j).EQ.f_ndflag.AND.
c     +        ft_sd_relh_pc(j).EQ.f_ndflag)) THEN
c           i_k1_reject=i_k1_reject+1
c          ENDIF
c         ENDIF
cc        Case: ppt
c         IF (k.EQ.2) THEN
c          IF (ft_sd_ppt_mm(j).EQ.f_ndflag.OR.
c     +        f_duration_s(j).EQ.f_ndflag) THEN
c           i_k2_retain=i_k2_retain+1
c          ENDIF
c          IF (.NOT.(ft_sd_ppt_mm(j).EQ.f_ndflag.OR.
c     +              f_duration_s(j).EQ.f_ndflag)) THEN
c           i_k2_reject=i_k2_reject+1
c          ENDIF
c         ENDIF
c*******
c        Pipe separators

c        Export line if any data elements
         IF (i_keep.EQ.1) THEN
         s_export_line=
...
3009     FORMAT(a400)
       ENDIF
c*******
c     Initial screen output column information
      GOTO 11
...
c      print*,'i_k1_retain,i_k1_reject=',i_k1_retain,i_k1_reject
c      print*,'i_k2_retain,i_k2_reject=',i_k2_retain,i_k2_reject
c************************************************************************
