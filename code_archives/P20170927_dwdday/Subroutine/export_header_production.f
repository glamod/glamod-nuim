...
c     AJ_Kettle, Nov27/2017
      ...
      SUBROUTINE export_header_production(f_ndflag,s_directory_root,
     +   s_stnnum_single,
...
c************************************************************************
      ...
      INTEGER             :: i_keep
c************************************************************************
      print*,'just entered export_header_production8'
...
c         s_time_single=s_dayavg_airt_stime(j)
c         s_hour  =s_time_single(1:2)
c         s_minute=s_time_single(3:4)
...
c        New rejection criteria
         i_keep=0
         IF (f_daytot_ppt_mm(j).EQ.f_ndflag.AND.
     +       f_dayavg_airt_k(j).EQ.f_ndflag.AND.
     +       f_daytot_snoacc_cm(j).EQ.f_ndflag.AND.
     +       f_dayavg_slpres_hpa(j).EQ.f_ndflag.AND.
     +       f_dayavg_wspd_ms(j).EQ.f_ndflag.AND.
     +       f_dayavg_relh_pc(j).EQ.f_ndflag) THEN
           i_reject=i_reject+1
         ENDIF         
         IF (.NOT.(f_daytot_ppt_mm(j).EQ.f_ndflag.AND.
     +       f_dayavg_airt_k(j).EQ.f_ndflag.AND.
     +       f_daytot_snoacc_cm(j).EQ.f_ndflag.AND.
     +       f_dayavg_slpres_hpa(j).EQ.f_ndflag.AND.
     +       f_dayavg_wspd_ms(j).EQ.f_ndflag.AND.
