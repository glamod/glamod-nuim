...
df['Timestamp'] = df['Timestamp'].dt.tz_localize('Etc/GMT+12').dt.tz_convert('GMT')
...
df["Observed_value2"]=df["Original_observed_value"]/10-100
df['Observed_value3'] = np.where(df['Observed_value2']>=50,+900,+1000)
df['Observed_value4'] = df['Observed_value2'] + df['Observed_value3'] 
df["Observed_value"]=df["Observed_value4"]
del df["Observed_value3"]  
del df["Observed_value2"]
del df["Observed_value4"]
#df = df.rename(columns=({'Observed_value4':'Observed_value'}))
#df["Observed_value"]= round(df["Observed_value"])


dfdf = df[["Source_ID",'Station_ID',"Station_name","Alias_station_name",  
...
with open('combined.csv') as fin:    
    ...
    for row in csvin:
        ...
        if cat not in outputs:
            fout = open ('{}_sea_level_pressure_244.psv'.format(cat), "w", newline = "")
