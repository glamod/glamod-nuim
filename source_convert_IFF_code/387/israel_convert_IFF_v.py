...
import glob
import pandas as pd
OUTDIR7 = "C:/Users/snoone/Dropbox/ISRAEL_HOURLY/IFF/v"
...
for filename in all_filenames:
    ...
    df["Source_QC_flag"]=""
    df["Original_observed_value_units"] = "(code)"
    df['Report_type_code'] = ""
    df['Measurement_code_1'] = ""
    df['Measurement_code_2'] = ""
    df = df.replace('null','', regex=True)
    df["Observed_value"] = df["Visibility (code)"]
    df["Original_observed_value"] = df["Visibility (code)"]
    try:
    ...
    try:
        ...
        outname = os.path.join(OUTDIR7, ID)
        a = df2['Station_ID'].unique()
        print (a)
        #with open(filename, "w") as outfile:
        df2.to_csv(outname+"_visibility_387.psv", index=False, sep="|")
    except:
        continue
