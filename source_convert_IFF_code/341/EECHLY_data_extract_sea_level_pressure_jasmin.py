...
os.chdir("/work/scratch-pw/snoone/HLY01/1953")
...
os.chdir(r"/work/scratch-pw/snoone/2011_21/mslp")
cats = sorted(result['Station_ID'].unique())
for cat in cats:
    outfilename = cat + "_sea_level_pressure_2011.csv"
    print(outfilename)
...
#merge with station details files
df2=pd.read_csv("/work/scratch-pw/snoone/HLY01/1953/Stations.csv")
df1 = df1.astype(str) 
df2 = df2.astype(str)
...
##write out separate pipe delimited files by station id to working directory namd by station_id+ variable name+sourceid need to change with each new source
os.chdir(r"/work/scratch-pw/snoone/2011_21/slp")
cats = sorted(result['Station_ID'].unique())
for cat in cats:
    outfilename = cat + "_station_pressure_2011.csv"
    print(outfilename)
    result[result["Station_ID"] == cat].to_csv(outfilename,sep=',',index=False)
...
##write out separate pipe delimited files by station id to working directory namd by station_id+ variable name+sourceid need to change with each new source
os.chdir(r"/work/scratch-pw/snoone/2011_21/ws")
cats = sorted(result['Station_ID'].unique())
for cat in cats:
    outfilename = cat + "_wind_speed_2011.csv"
    print(outfilename)
    result[result["Station_ID"] == cat].to_csv(outfilename,sep=',',index=False)
...
##write out separate pipe delimited files by station id to working directory namd by station_id+ variable name+sourceid need to change with each new source
os.chdir(r"/work/scratch-pw/snoone/2011_21/dpt")
cats = sorted(result['Station_ID'].unique())
for cat in cats:
    outfilename = cat + "_dew_point_temperature_2011.csv"
    print(outfilename)
    result[result["Station_ID"] == cat].to_csv(outfilename,sep=',',index=False)
###########################################################temperature
...
##write out separate pipe delimited files by station id to working directory namd by station_id+ variable name+sourceid need to change with each new source
os.chdir(r"/work/scratch-pw/snoone/2011_21/tt")
cats = sorted(result['Station_ID'].unique())
for cat in cats:
    outfilename = cat + "_temperature_2011.csv"
    print(outfilename)
    result[result["Station_ID"] == cat].to_csv(outfilename,sep=',',index=False)    
...
##write out separate pipe delimited files by station id to working directory namd by station_id+ variable name+sourceid need to change with each new source
os.chdir(r"/work/scratch-pw/snoone/2011_21/rh")
cats = sorted(result['Station_ID'].unique())
for cat in cats:
    outfilename = cat + "_relative_humidity_2011.csv"
    print(outfilename)
    result[result["Station_ID"] == cat].to_csv(outfilename,sep=',',index=False)    
