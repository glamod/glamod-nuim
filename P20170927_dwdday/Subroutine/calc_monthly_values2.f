...
c     AJ_Kettle, Nov09/2017
      ...
      SUBROUTINE calc_monthly_values2(f_ndflag,i_ndflag,
     ...
     +   f_monrec_nseconds,
     +   s_monrec_stime,s_monrec_timezone,
     +   f_monrec_totalvalue)
...
c      REAL                :: f_daytot_ppt_mm(100000)
c      REAL                :: f_dayavg_wspd_ms(100000)
c      REAL                :: f_dayavg_pres_mb(100000)
      ...
      CHARACTER(LEN=8)    :: s_matrix_stime(300,12)
      CHARACTER(LEN=5)    :: s_matrix_timezone(300,12)
      ...
      REAL                :: f_monrec_value(5000)       !month average 
      INTEGER             :: i_monrec_flag(5000)
      REAL                :: f_monrec_nseconds(5000)
      REAL                :: f_monrec_ndays(5000)
      CHARACTER(LEN=8)    :: s_monrec_stime(5000)
      CHARACTER(LEN=5)    :: s_monrec_timezone(5000)
      REAL                :: f_monrec_totalvalue(5000)  !month total

      INTEGER             :: ist,ien
c************************************************************************
c      print*,'dayavg_values=',(f_dayavg_airt_c(i),i=1,10)
c      print*,'stime=',(s_dayavg_airtk_stime(i),i=1,10)
c      print*,'timezone=',(s_dayavg_airtk_timezone(i),i=1,10)
c      CALL SLEEP(5)
...
c       Archive time & timezone
        s_matrix_stime(i_loc_year,i_loc_month)=
     +    s_dayavg_airtk_stime(i)
        s_matrix_timezone(i_loc_year,i_loc_month)=
     +    s_dayavg_airtk_timezone(i)
...
c       number of seconds in month
        f_monrec_ndays(ii)=REAL(i_matrix_monthdays(j,k))
        f_monrec_nseconds(ii)=f_monrec_ndays(ii)*(24.0*3600.0)

c       Initialize series with ndflag
        f_monrec_value(ii)     =f_ndflag
        f_monrec_totalvalue(ii)=f_ndflag 
        i_monrec_flag(ii)      =i_ndflag

c       Initialize time & timezone
        s_monrec_stime(ii)   =''
        s_monrec_timezone(ii)=''

        IF (i_matrix_monthflag(j,k).EQ.0.OR.
     +      i_matrix_monthflag(j,k).EQ.1) THEN

         f_monrec_value(ii)     =f_matrix_average(j,k)
         f_monrec_totalvalue(ii)=f_matrix_integrator(j,k)
         i_monrec_flag(ii)      =i_matrix_monthflag(j,k)

c        Time & timezone
         s_monrec_stime(ii)   =s_matrix_stime(j,k)
         s_monrec_timezone(ii)=s_matrix_timezone(j,k)
