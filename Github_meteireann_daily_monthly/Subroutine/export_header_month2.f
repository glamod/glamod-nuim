...
c     AJ_Kettle, Dec24/2017
      ...
      SUBROUTINE export_header_month2(f_ndflag,s_directory_root,
...
c************************************************************************
      print*,'just inside export_header_month2'
...
c         s_date_single=s_vec_date(j)
         ...
         s_time_single=s_moncom_stime(j)
...
c        Jan12,2018: time zone changed from 5 to 3 character format
...
c*******
c        Establish condition for keeping record

         i_keep=0
         IF (f_moncom_tot_rain_mm(j).EQ.f_ndflag.AND.
     +       f_moncom_airt_c(j)     .EQ.f_ndflag.AND.
     +       f_moncom_airt_k(j)     .EQ.f_ndflag.AND.
     +       f_moncom_max_maxdy_c(j).EQ.f_ndflag.AND.
     +       f_moncom_max_maxdy_k(j).EQ.f_ndflag.AND.
     +       f_moncom_min_mindy_c(j).EQ.f_ndflag.AND.
     +       f_moncom_min_mindy_k(j).EQ.f_ndflag) THEN
           i_reject=i_reject+1
         ENDIF         
         IF (.NOT.(f_moncom_tot_rain_mm(j).EQ.f_ndflag.AND.
     +       f_moncom_airt_c(j)     .EQ.f_ndflag.AND.
     +       f_moncom_airt_k(j)     .EQ.f_ndflag.AND.
     +       f_moncom_max_maxdy_c(j).EQ.f_ndflag.AND.
     +       f_moncom_max_maxdy_k(j).EQ.f_ndflag.AND.
...
c         i_keep=0
c         IF (f_monrec_totrain_mm(j).EQ.f_ndflag)THEN
c           i_reject=i_reject+1
c         ENDIF         
c         IF (.NOT.(f_monrec_totrain_mm(j).EQ.f_ndflag)) THEN
c           i_retain=i_retain+1
c           i_keep=1
c         ENDIF        
c*******
         IF (i_keep.EQ.1) THEN
