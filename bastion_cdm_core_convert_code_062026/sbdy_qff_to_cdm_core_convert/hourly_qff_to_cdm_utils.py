...
def apply_wind_measurement_codes(var_frame, retained_measurement_codes_list):
    ...
    for c, code in enumerate(retained_measurement_codes_list):
        if c == 0:
            # Initialise
            if code == "":
                mask = var_frame["measurement_code"].isna()
            else:
                mask = (var_frame["measurement_code"] == code)

        else:
            # Combine using or (|)
            #   Hence if code = e.g. "N-Normal" or "C-Calm" or "" set True
            if code == "":
                mask = var_frame["measurement_code"].isna() | mask
            else:
                mask = (var_frame["measurement_code"] == code) | mask


	# TODO - sort this startswith() call - fails if all entries are NaNs
#        else:
