...
df = pd.read_csv("DWR_UKMO_DWRUK_LONDON_18991231-19041229_ta.tsv",skiprows=12, sep='\t')
#extract the first 11 rows of the header to a df
df1=pd.read_csv("DWR_UKMO_DWRUK_LONDON_18991231-19041229_ta.tsv", nrows=11,sep='\t', squeeze=True)
df2=df1.iloc[:,0:2]
...
with open('output.csv') as fin:    
    ...
    for row in csvin:
        ...
        if cat not in outputs:
            fout = open ('{}_temperature_316.psv'.format(cat), "w", newline = "")##change source ID
