...
for filename in all_filenames:
    ...
    df["Observed_value"] = df["Wind direction (°)"]
    df["Original_observed_value"] = df["Wind direction (°)"]
    try:
    ...
    try:
        ...
        outname = os.path.join(OUTDIR5, ID)
        a = df2['Station_ID'].unique()
        print (a)
        #with open(filename, "w") as outfile:
        df2.to_csv(outname+"_wind_direction_387.psv", index=False, sep="|")
    except:
        continue