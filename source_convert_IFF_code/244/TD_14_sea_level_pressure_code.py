...
df = pd.read_csv("header.csv", usecols = ['Year', 'Month',"Day",
                                          ...
                                          "Longitude",
                                          "Latitude","Elevation",              
                                          "Station number","Sea level pressure"])
...
df['Original_observed_value_units']="mb_tenths"
...
df = df.rename(columns=({'Sea level pressure':'Observed_value'}))
...
df['Observed_value2']=df["Observed_value"]/100
