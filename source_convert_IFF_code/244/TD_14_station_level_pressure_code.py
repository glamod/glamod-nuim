...
df = pd.read_csv("header.csv", usecols = ['Year', 'Month',"Day",
                                          ...
                                          "Longitude",
                                          "Latitude","Elevation",              
                                          "Station number","Station pressure"])
...
df['Original_observed_value_units']="inhg_hudths"
...
df = df.rename(columns=({'Station pressure':'Observed_value'}))
...
df['Observed_value3']=df["Observed_value2"]* 33.86389 
...
df = df.rename(columns=({'Observed_value3':'Observed_value'}))
