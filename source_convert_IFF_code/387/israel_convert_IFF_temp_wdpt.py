...
for filename in all_filenames:
    ...
    df["Observed_value"] = df["Dew point temperature (°C)"]
    df["Original_observed_value"] = df["Dew point temperature (°C)"]
    try:
    ...
    try:
        ...
        outname = os.path.join(OUTDIR3, ID)
        a = df2['Station_ID'].unique()
        print (a)
        #with open(filename, "w") as outfile:
        df2.to_csv(outname+"_dew_point__temperature_387.psv", index=False, sep="|")
    except:
        continue