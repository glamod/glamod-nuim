...
c     AJ_Kettle, 08Jan2019
      ...
      SUBROUTINE convert_float_to_string2(f_ndflag,s_format,      
...
c     Variables used in program
      ...
      INTEGER             :: i_len
c************************************************************************
c      print*,'just entered convert_float_to_string'

      DO i=1,l_timestamp
       s_prcp_datavalue_mm(i)=''    !initialize string to blank
       IF (f_prcp_datavalue_mm(i).NE.f_ndflag) THEN 
        WRITE(s_string,s_format) f_prcp_datavalue_mm(i)
        s_prcp_datavalue_mm(i)=ADJUSTL(TRIM(s_string))
        ...
        IF (i_len.GE.10) THEN 
         ...
         STOP 'convert_float_to_string; string too long'
        ENDIF
       ENDIF
      ENDDO

c      print*,'just leaving convert_float_to_string'
