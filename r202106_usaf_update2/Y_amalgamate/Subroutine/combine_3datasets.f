...
c     AJ_Kettle, 26Apr2021
      ...
      SUBROUTINE combine_3datasets(s_date_st,s_time_st,
...
c       Initialize vector
        ...
        IF (j.EQ.1) THEN
         s_directory_dataroot=s_directory_dataroot1
        ENDIF 
        IF (j.EQ.2) THEN
         s_directory_dataroot=s_directory_dataroot2
        ENDIF 
        IF (j.EQ.3) THEN
         s_directory_dataroot=s_directory_dataroot3
        ENDIF 
...
c        Input lines single file
         CALL input_lines_1collection(s_directory_dataroot,
     +    s_subdirectory_sorted,
     +    s_single_stn,
...
c        Output line collection
         CALL output_lines_1collection(s_directory_assemstns,
     +    s_single_stn,
     +    l_rgh_char,l_rgh_datalines,
     +    l_lines,s_linsto2,
     +    s_header)
...
c      Output data line record
       CALL export_stnlist_linecount(s_date_st,s_time_st,
     +  s_directory_diagnostics,s_file_linecount,
     +  s_single_stn,l_vec_lines, 
     +  l_amal)

       print*,'i,i_mat_stnlist_flag=',i,(i_mat_stnlist_flag(i,j),j=1,3)
