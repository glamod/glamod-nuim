# -*- coding: utf-8 -*-
"""
Created on Wed Feb 26 13:06:44 2025
...
def read_excel_files(directory, filenames):
    ...
    for filename in filenames:
        ...
        if df is not None:
            # Restrict to 15 columns and name them
            df = df.iloc[:, :15]  # Restrict the DataFrame to the first 15 columns
    ...
    for filename in data1:
        if filename in data2:
            ...
            if df1.shape != df2.shape:
                mismatched_info.append(f"File '{filename}': Shapes do not match ({df1.shape} vs {df2.shape})\n")
                continue
            ...
            for row in range(df1.shape[0]):
                for col in range(df1.shape[1]):
                    ...
                    if (
                        ...
                        (val1 in ["Calme"] and val2 in ["Calm"]) or
                        (val2 in ["Calme"] and val1 in ["Calm"])or
                        (val1 in ["Cal"] and val2 in ["cal"]) or
                        (val2 in ["Cal"] and val1 in ["cal"])or
                        (val1 in ["cal "] and val2 in ["cal"]) or
                        (val2 in ["cal "] and val1 in ["cal"])or
                        (val1 in ["cal "] and val2 in ["CAL"]) or
                        (val2 in ["cal "] and val1 in ["CAL"])or
                        (val1 in ["cal "] and val2 in ["CALME"]) or
                        (val2 in ["cal "] and val1 in ["CALME"])or
                        (val1 in ["cal"] and val2 in ["CAL"]) or
                        (val2 in ["cal"] and val1 in ["CAL"])or
                        (val1 in ["cal"] and val2 in ["CALME"]) or
                        (val2 in ["cal"] and val1 in ["CALME"])
                    ):
                        continue
...
def log_unmatched_files(directory1, directory2, unmatched_file):
    ...
    unmatched_files += [f"File '{filename}' not in directory2\n" for filename in files1 if filename not in files2]
    unmatched_files += [f"File '{filename}' not in directory1\n" for filename in files2 if filename not in files1]
...
if __name__ == "__main__":
    directory1 = 'C:/Users/snoone/Dropbox/OLD_BELGIAN_AFRICAN_DARE_INVENTORY/data_for_transcribing_2023/completed_kentucky_data_rescue_2025/completed_forms/file_A/form3'
    directory2 = 'C:/Users/snoone/Dropbox/OLD_BELGIAN_AFRICAN_DARE_INVENTORY/data_for_transcribing_2023/completed_kentucky_data_rescue_2025/completed_forms/file_B/form3'
    output_file = 'C:/Users/snoone/Dropbox/OLD_BELGIAN_AFRICAN_DARE_INVENTORY/data_for_transcribing_2023/completed_kentucky_data_rescue_2025/completed_forms/marolambo_mismatched_form3.txt'
    unmatched_file = 'C:/Users/snoone/Dropbox/OLD_BELGIAN_AFRICAN_DARE_INVENTORY/data_for_transcribing_2023/completed_kentucky_data_rescue_2025/completed_forms/marolambo_unmatched_form3.txt'
