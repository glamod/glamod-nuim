...
df = pd.read_csv("K4552K.csv", usecols = ['Year', 'Month',"Day",
                                          "Hour (LST)","Greenwich offset",
                                          "Longitude",
                                          "Latitude","Elevation",              
                                          "Station number","Wind speed"])
...
df = df.rename(columns=({'Wind speed':'Observed_value'}))
...
with open('output2.csv') as fin:    
    ...
    for row in csvin:
        ...
        if cat not in outputs:
            fout = open ('{}_wind_speed_244.psv'.format(cat), "w", newline = "")
