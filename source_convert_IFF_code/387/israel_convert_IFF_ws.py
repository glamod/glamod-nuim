...
for filename in all_filenames:
    ...
    df["Observed_value"] = df["Wind speed (m/s)"]
    df["Original_observed_value"] = df["Wind speed (m/s)"]
    try:
    ...
    try:
        ...
        outname = os.path.join(OUTDIR6, ID)
        a = df2['Station_ID'].unique()
        print (a)
        #with open(filename, "w") as outfile:
        df2.to_csv(outname+"_wind_speed_387.psv", index=False, sep="|")
    except:
        continue
