c     Subroutine to export monthly observation
c     AJ_Kettle, Dec25/2017

      SUBROUTINE export_observation_month3(f_ndflag,s_directory_root,
...
c************************************************************************
      ...
      CHARACTER(LEN=3)    :: s_source_id                !49
      CHARACTER(LEN=39)   :: s_multiple_source          !50

      INTEGER             :: i_reject
      INTEGER             :: i_retain
      INTEGER             :: i_keep
c************************************************************************
      print*,'just entered export_observation_month'
...
c     Declare variable codes
      s_code(1) ='44'    !ppt
      s_code(2) ='85'    !airt_max
      s_code(3) ='85'    !airt_min
      s_code(4) ='85'    !airt_avg

      s_value_signif_code(1)='13'
      s_value_signif_code(2)='0'
      s_value_signif_code(3)='1'
...
c        Output header here - full length titles
     ...
     + TRIM('source_id')//'|'//TRIM('source_record_id')
c*****
...
c          s_date_single=s_vec_date(j)
          s_year =s_moncom_year(j)
          s_month=s_moncom_month(j)
          s_day  ='01'
...
c         1=only original value in non-si units available; no conversion performed
c         2=orginal value in si-units; no conversion required
...
c         40.s_original_value
          IF (k.EQ.1) THEN
           ...
           IF (s_conversion_flag.NE.'2') THEN 
            f_test=f_moncom_tot_rain_mm(j)               !ppt_mm
            IF (f_test.EQ.f_ndflag) THEN 
...
c         48.s_advanced_homogen
          s_adv_homogen='0'
c*******
c         49.s_source_id (DWD=82; Meteiranne=158)
          s_source_id='158'
c*******
c         50.s_multiple_source
          s_multiple_source=TRIM(s_source_id)//'-'//TRIM(s_report_id)
c*******
c s_moncom_month(l_mlent_mon)
c s_moncom_year(l_mlent_mon)
c f_moncom_nseconds(l_mlent_mon)
c s_moncom_stime(l_mlent_mon)
c s_moncom_timezone5(l_mlent_mon)
c f_moncom_tot_rain_mm(l_mlent_mon)
c f_moncom_max_maxdy_k(l_mlent_mon)
c f_moncom_min_mindy_k(l_mlent_mon)
c f_moncom_airt_k(l_mlent_mon)
...
c*******
     ...
     + TRIM(s_source_id)//'|'//TRIM(s_multiple_source)
...
c*******
c************************************************************************
      print*,'just leaving export_observation_month'
