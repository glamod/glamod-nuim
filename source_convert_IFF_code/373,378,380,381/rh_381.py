...
OUTDIR = "D:/Ed_Hawkins_data/ehawkins/UK-DWRs/IFF/rh"
os.chdir("D:/Ed_Hawkins_data/ehawkins/UK-DWRs/sef_data/rh")
extension = 'tsv'
...
for filename in all_filenames:
    ...
    #df['Original_observed_value'] = df['Original_observed_value'].map(lambda x: x.lstrip('orig=').rstrip('in'))
            #reorder headings
...
os.chdir("D:/Ed_Hawkins_data/ehawkins/UK-DWRs/IFF/rh")
extension = 'tsv'
all_filenames = [i for i in glob.glob('*.{}'.format(extension))]
...
with open('combined.csv') as fin:    
    ...
    for row in csvin:
        ...
        if cat not in outputs:
            fout = open ('{}_relative_humidity_381.psv'.format(cat), "w", newline = "")
