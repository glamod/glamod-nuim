...
########################################################################################################
# change Source_ID to C3S number need to change with each new source
giantDataFrame["Source_ID"] = 268


# and fix longitudes to -180 --> 180
giantDataFrame["Longitude"] = giantDataFrame["Longitude"].astype(float)

locs, = np.where(np.logical_and( 180 < giantDataFrame["Longitude"], giantDataFrame["Longitude"] <= 360))

giantDataFrame["Longitude"][locs] = giantDataFrame["Longitude"][locs] - 360.
giantDataFrame['Longitude']= round(giantDataFrame['Longitude'],2)
    
giantDataFrame["Longitude"].astype(str)

############################################################
##write out separate pipe delimited files by station id to working directory named by station_id+ variable name+sourceid need to change with each new source
...
for station in stationList:
    ...
    currentDataFrame = giantDataFrame[giantDataFrame['Station_ID'] == station]
     # test if number of Nulls is the same length as the Data frame
    locs, = np.where(currentDataFrame["Observed_value"] == "Null")
    if locs.shape[0] == currentDataFrame["Observed_value"].shape[0]:
        print("All observed values in {} are Null.  No file created".format(station))
    else:       
        currentDataFrame.to_csv(os.path.join(os.curdir, "output/station_pressure", station + "_station_level_pressure_268.psv"),sep="|",index=False)
