...
# -*- coding: utf-8 -*-
"""
Convert OBS files to CDM HEADER .psv files (one per station).
...
def main(station="", subset="", run_all=False, clobber=False):
    """
    Run processing of hourly cdm obs to CDM header file
    ...
    if station != "":
        ...
        all_filenames = [os.path.join(utils.SUBDAILY_HEAD_IN_DIR, f"{utils.SUBDAILY_CDM_OBS_FILE_ROOT}{station}.{EXTENSION}")]
    elif subset != "":
        ...
        try:
            with open(subset, "r") as f:
                ...
                for infile in filenames:
                    all_filenames += [os.path.join(utils.SUBDAILY_HEAD_IN_DIR, f"{utils.SUBDAILY_CDM_OBS_FILE_ROOT}{infile}.{EXTENSION}")]
    ...
    for filename in all_filenames:
        ...
        # sort, and then format strings
        hdf.sort_values("report_timestamp", inplace=True)
