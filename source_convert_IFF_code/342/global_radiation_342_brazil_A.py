...
OUTDIR = "D:/brazil_inmet/DataA_all_years_IFF/global_radiation/"
...
for filename in all_filenames:
    ...
    #df3 = pd.read_csv("D:/brazil_inmet/inventory/header.csv",index_col="REGIAO:",squeeze=True)#odd
    ...
    df['Minute'] = "0"
    df["Source_QC_flag"]=""
    df["Original_observed_value_units"] = "KJ/M2"
    df['Report_type_code'] = ""
    ...
    #df['Hour'] = df['HORA (UTC)']
    #df["Original_observed_value"] = df["RADIACAO GLOBAL (KJ/m²)"]
    #df["Observed_value"] = df["RADIACAO GLOBAL (KJ/m²)"]
    df["Original_observed_value"] = df["RADIACAO GLOBAL (KJ/m²)"]#odd
    df["Observed_value"] = df["RADIACAO GLOBAL (KJ/m²)"]#odd
    df = df.astype({"Hour": int})
    df["Hour"]=df["Hour"]/100 #odd 2019
    ...
    df3.to_csv(outname+"_global_radiation_342.psv", index=False, sep="|")
