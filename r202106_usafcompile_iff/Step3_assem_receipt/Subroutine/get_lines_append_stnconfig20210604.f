...
c     27Mar2020: modified for USAF update
      ...
      SUBROUTINE get_lines_append_stnconfig20210604(
...
c     Cycle through station files
      ...
      DO i=1,l_stn     !cycle through receipt files

c      get separated netplat for receipt
       s_single_receipt=s_filelist(i)
       ...
       IF (s_single_receipt(1:1).EQ.'W') THEN 
        ...
        GOTO 10
       ENDIF
       print*,'no netplat found receipt',TRIM(s_single_receipt)
 10    CONTINUE

c       print*,'np receipt='//TRIM(s_net_rec)//'='//TRIM(s_plat_rec)//'='

c      Cycle through actual files
       i_testgood=0     !set to bad
       DO j=1,l_iffuniq
        s_single_filelist=TRIM(s_vec_iffuniq_netplat(j))
...
c        Can continue compilation process if here
...
c        print*,'receipt without actual file; skip receipt assemble'
...
c       print*,'very bad='//TRIM(s_net_rec)//'='//TRIM(s_plat_rec)
c       STOP 'get_lines_append_stnconfig20210604'
