...
def main(station="", subset="", run_all=False, clobber=False):
    ...
    # Read in the height_of_station_above_sea_level dataframe (only read in if needed)
    height_of_station_above_sea_level_df = pd.read_csv(utils.DAILY_STATION_RECORD_ENTRIES_OBS_LITE, encoding='latin-1')
    height_of_station_above_sea_level_df = height_of_station_above_sea_level_df.astype(str)
    ...
    for filename in all_filenames:
        ...
        df["Seconds"] = "00"
        df["offset"] = "+00"
        df["report_timestamp"] = df["Timestamp2"].map(str)+ " " + df["hour"].map(str) + ":" + \
                          df["Minute"].map(str)+":"+df["Seconds"].map(str) 
        df.report_timestamp = df.report_timestamp + '+00'
        ...
        # add data height of station above sea level information
        #df = d_utils.add_height_above_sea_level(df, height_of_station_above_sea_level_df)
        ...
        df["dates"] = ""
        df["dates"] = df["report_id_a"].str[:-11]
        ...
        df["report_id"] = df['primary_station_id'] + '-' +df['record_number'].astype(str) + '-' + df['dates'].astype(str)
        df['report_id'] = df['report_id'].str.rstrip()
        ...
        # reorder df columns
        df = df[["station_name","primary_station_id","report_id","observation_id","longitude","latitude","height_of_station_above_sea_level",
                 "report_timestamp","report_meaning_of_time_stamp","report_duration","observed_variable","units","observation_value"
                 ,"quality_flag","source_id","data_policy_licence","report_type",
                  "value_significance"]]
