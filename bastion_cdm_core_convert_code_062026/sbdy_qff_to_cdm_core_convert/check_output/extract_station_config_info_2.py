...
>python3 extract_station_config_info.py --station STATIONID --time H --clobber
>python3 extract_station_config_info.py --subset FILENAME --time D
>python3 extract_station_config_info_2.py --run_all --time D --clobber
>python3 extract_station_config_info.py --help
...
def main(station="", subset="", run_all=False, clobber=False, time=""):
    ...
    if time not in ["H", "D", "M"]:
        ...
        compression = ".gz"
        ##change path
        indir = utils.SUBDAILY_CDM_CORE_OUT_DIR
        # inroot = utils.SUBDAILY_CDM_CORE_FILE_ROOT
        outname = f"{indir}/sub_daily_station_config_5.dat"
    elif time == "D":
        extension = ".psv"
        compression = ".gz"
        indir = utils.DAILY_CDM_CORE_OUT_DIR
        # inroot = utils.DAILY_CDM_CORE_FILE_ROOT
        outname = f"{indir}/daily_station_config.dat"
    elif time == "M":
        extension = ".psv"
        compression = ""
        indir = utils.MONTHLY_CDM_CORE_OUT_DIR
        # inroot = utils.MONTHLY_CDM_CORE_FILE_ROOT
