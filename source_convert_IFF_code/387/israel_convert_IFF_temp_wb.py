...
for filename in all_filenames:
    ...
    df["Observed_value"] = df["Wet temperature (°C)"]
    df["Original_observed_value"] = df["Wet temperature (°C)"]
    try:
    ...
    try:
        ...
        outname = os.path.join(OUTDIR2, ID)
        a = df2['Station_ID'].unique()
        print (a)
        #with open(filename, "w") as outfile:
        df2.to_csv(outname+"wet_bulb_temperature_387.psv", index=False, sep="|")
    except:
        continue