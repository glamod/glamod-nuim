...
for filename in all_filenames:
    ...
    stn_list = pd.read_csv("D:/Ed_Hawkins_data/weather-rescue-data/MET-NO-ARCHIVE_new/Stations.csv").astype(str)
   ...
   #os.chdir("D:/Ed_Hawkins_data/ehawkins/MET-NO-ARCHIVE/IFF/slp/out")    
    cats = sorted(df['Station_ID'].unique())
    for cat in cats:
        outfilename = cat +"_station_level_pressure_378.psv"
        print(outfilename)
        df[df["Station_ID"] == cat].to_csv(outfilename,sep='|',index=False)
       # os.chdir("D:/Ed_Hawkins_data/ehawkins/MET-NO-ARCHIVE/IFF/slp")
