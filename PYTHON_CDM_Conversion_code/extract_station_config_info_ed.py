...
>python3 extract_station_config_info_ed.py --station STATIONID --time H 
>python3 extract_station_config_info_ed.py --subset FILENAME --time D
>python3 extract_station_config_info_ed.py --run_all --time H 
...
def main(station="", subset="", run_all=False, clobber=False, time=""):
    ...
    if time not in ["H", "D", "M"]:
        ...
        compression = ".gz"
        ##change path
        indir = utils.EXTRACTION_FILE_PATH
        # inroot = utils.SUBDAILY_CDM_CORE_FILE_ROOT
        outname = f"{indir}/sub_daily_station_config_ed.dat"
    elif time == "D":
        extension = ".psv"
        compression = ".gz"
